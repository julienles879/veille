...
class UpdateLastViewedView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not article_id:
            return Response(
                {"error": "Article ID is required."}, status=HTTP_404_NOT_FOUND
            )
        try:
            article = RSSFeedEntry.objects.get(id=article_id)
            article.update_last_viewed()  # ✅ Met à jour last_viewed_at
            return Response(
                {"message": "Dernière consultation mise à jour."},
                status=HTTP_200_OK,
            )
        except RSSFeedEntry.DoesNotExist:
            return Response(
                {"error": "Article not found."}, status=HTTP_404_NOT_FOUND
            )
...
class AddFavoriteView(APIView):
    """
    Vue pour ajouter un article aux favoris.
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"message": "Article is already a favorite."},
